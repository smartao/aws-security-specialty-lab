...
**Status:** 🟡 Terraform e bootstrap implementados, ainda não aplicado nesta sessão (depende do Lab 01 estar de pé — ver Implementação).
...
- **Root account usage:** `aws_cloudwatch_log_metric_filter` com o pattern padrão CIS + `aws_cloudwatch_metric_alarm` + `aws_sns_topic` (assinatura por e-mail opcional via `var.alarm_notification_email`, vazia por padrão).
...
- `terraform fmt` e `terraform validate` limpos.

### AWS CLI (validação pós-apply)
_(a definir — validação de campo a campo será feita após o primeiro `apply`, seguindo o mesmo padrão do Lab 01: `describe-trail`, `get-trail-status`, `describe-flow-logs`, consulta ao log group via `filter-log-events`)_

## Testes
_(a definir — pendente do primeiro apply, que depende do Lab 01 estar reaplicado nesta sessão)_
...
_(a definir — depende do apply)_
